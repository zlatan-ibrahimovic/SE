...
{
  // CPU side
  A_data = malloc(10 * sizeof(int));
  B_data = malloc(10 * sizeof(int));
  ...
  //
  A_buffer = clCreateBuffer(context,  CL_MEM_READ_WRITE,  sizeof(int) * 10, NULL, NULL);
  if (!A_buffer)
    error("Failed to allocate input buffer A");

  B_buffer = clCreateBuffer(context,  CL_MEM_READ_WRITE,  sizeof(int)  * 10, NULL, NULL);
  if (!B_buffer)
...
static void send_input(cl_command_queue queue)
{
  ...
  err = clEnqueueWriteBuffer(queue, A_buffer, CL_TRUE, 0,
			     sizeof(int) * 10, A_data, 0, NULL, NULL);
...
static void retrieve_output(cl_command_queue queue)
{
  ...
  err = clEnqueueReadBuffer(queue, B_buffer, CL_TRUE, 0,
			    sizeof(int) * 10, B_data, 0, NULL, NULL );  
...
int main(int argc, char** argv)
{
  ...
  for (cl_int p = 0; p < nb_platforms; p++) {
	...
	struct timeval t1,t2;
	double timeInMicroseconds;
	size_t global = 10;  // global domain size for our calculation
	...
	err |= clSetKernelArg(kernel, 1, sizeof(cl_mem), &B_buffer);
       	err |= clSetKernelArg(kernel, 2, sizeof(int), &n_buffer);
